...
func _set_labels(team: Team) -> void:
	...
	budget_label.text = tr("BUDGET") + " " + FormatUtil.currency(team.finances.balance)
	salary_budget_label.text = tr("SALARY_BUDGET") + " " + FormatUtil.currency(team.finances.get_salary_budget())
