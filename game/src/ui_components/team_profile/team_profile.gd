...
func _set_labels(team: Team) -> void:
	...
	budget_label.text = tr("BUDGET") + " " + FormatUtil.get_sign(team.finances.balance)
	salary_budget_label.text = tr("SALARY_BUDGET") + " " + FormatUtil.get_sign(team.finances.get_salary_budget())
